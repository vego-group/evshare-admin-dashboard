...
export type ReportFilterOption<T extends string> = {
  label: string;
  value: T;
};

type ReportFilterSelectProps<T extends string> = {
  label: string;
...
function ReportFilterSelect<T extends string>({
  label,
  options,
  value,
  onChange,
}: ReportFilterSelectProps<T>) {
    ...
    <div className="relative h-9.5 w-full text-sm font-medium leading-5 text-dark-gray sm:w-49">
      ...
      {isOpen ? (
        <div
          role="listbox"
          aria-label={label}
          className="absolute right-0 top-[calc(100%+2px)] z-30 w-full rounded-[14px] border border-primary bg-bg-warm-ivory shadow-[0_10px_24px_rgba(16,24,40,0.12)]"
        >
