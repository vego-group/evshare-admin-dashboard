...
import ReportsContentShimmer, {
  ReportsHistoryShimmer,
} from "./content-shimmer";
...
function filterOptions(
  ...
  return inRule
    ? inRule
        .slice(3)
        .split(",")
        .map((value) => [value.trim(), value.trim()])
    : null;
}
    ...
    >
      <div className="mb-5">
        ...
        <p className="mt-2 text-xs text-gray">
          الإصدار {report.version} · {report.grouping}
        </p>
      </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {Object.entries(report.filters).map(([name, definition]) => {
            ...
            const isDate =
              name === "date_from" ||
              name === "date_to" ||
              definition.rules?.some((rule) => rule.startsWith("date_format:"));
            return (
              <label
                key={name}
                className="flex flex-col gap-1.5 text-sm font-medium text-dark-gray"
              >
                <span>
                  {fieldLabels[name] || name.replaceAll("_", " ")}
                  {definition.required && " *"}
                </span>
                {options ? (
                  <select
...
async function downloadReport(id: string) {
  const response = await fetch(
    `/api/admin/reports/exports/${encodeURIComponent(id)}/download`,
  ...
  link.href = blobUrl;
  link.download =
    response.headers.get("X-Export-Filename") || `report-${id}.csv`;
...
export default function ManagementReports() {
  ...
  const catalogQuery = useQuery({
    queryKey: ["reports-catalog", tenant],
    queryFn: reportCatalogAPI,
  });
  ...
  useEffect(() => {
    ...
    if (
      ...
      void downloadReport(active.id).then(
        ...
        (error) =>
          toast.error(
            error instanceof Error ? error.message : "تعذر تنزيل التقرير",
          ),
      );
    }
    ...
    <div className="flex w-full flex-col gap-6" dir="rtl">
      <header>
        <h1 className="flex items-center gap-2 text-2xl font-bold text-secondary">
          <FileBarChart2 className="size-6" /> التقارير الإدارية
        </h1>
        <p className="mt-1 text-sm text-gray">
          أنشئ تقارير CSV وتابع حالة تجهيزها وتنزيلها.
        </p>
      </header>
      <div
      ...
      {activeId && (
        <div
          role="status"
          className="rounded-2xl border border-primary/20 bg-white p-4 text-sm text-dark-gray"
        >
          {statusQuery.isError ? (
            <span className="text-red-700">
              تعذر متابعة حالة التقرير. يمكنك تحديث السجل للاطلاع عليه لاحقاً.
            </span>
          ) : active ? (
            <div className="flex flex-wrap items-center gap-3">
              ...
              {active.error && (
                <span className="text-red-700">{active.error}</span>
              )}
              {active.status === "completed" && (
                <PermissionGate slug={ADMIN_PERMISSIONS.reports.download}>
                  <Button
                    size="sm"
                    variant="outline"
                  ...
                  >
                    <Download /> تنزيل مجدداً
                  </Button>
                </PermissionGate>
              )}
            </div>
          ) : (
            "جارٍ متابعة حالة التقرير..."
          )}
        </div>
      )}
      {tab === "catalog" ? (
        <div className="space-y-5">
            ...
            {reports.map((report) => (
              ...
              >
                ...
                <p className="mt-4 text-xs text-gray">
                  الإصدار {report.version} · {report.format.toUpperCase()}
                </p>
              </button>
            ))}
          </div>
          {!canGenerate && reports.length > 0 && (
            <p className="text-sm text-gray">
              تحتاج إلى صلاحية إنشاء التقارير لطلب ملف جديد.
            </p>
          )}
          {selected && canGenerate && (
            <ReportForm
              key={`${tenant}:${selected.key}`}
              report={selected}
              busy={Boolean(
                activeId &&
                (!active ||
        ...
        <section className="space-y-4 rounded-2xl bg-white p-5 shadow-sm">
          ...
          {!!exports.length && (
            <div className="overflow-x-auto rounded-xl border border-primary/15">
              <table className="w-full min-w-170 text-right text-sm">
                    ...
                    <tr key={item.id} className="border-t border-primary/15">
                      <td className="px-4 py-3">
                        <div className="font-medium text-dark-gray">
                          {reports.find((report) => report.key === item.type)
                            ?.title || item.type}
                        </div>
                        <div
                          className="max-w-56 truncate text-xs text-gray"
                          dir="ltr"
                        >
                      ...
                      <td className="px-4 py-3">
                        {item.row_count ?? item.processed_rows ?? "—"}
                      </td>
                      <td className="px-4 py-3">
                        <span className="rounded-full bg-primary/10 px-3 py-1 text-secondary">
                          {statusLabels[item.status] || item.status}
                        </span>
                        {item.progress != null &&
                          item.status === "processing" && (
                            <span className="mr-2 text-xs text-gray">
                      ...
                      <td className="px-4 py-3">
                        {item.status === "completed" && (
                          <PermissionGate
                            slug={ADMIN_PERMISSIONS.reports.download}
                          >
                            <Button
                              type="button"
                              size="sm"
