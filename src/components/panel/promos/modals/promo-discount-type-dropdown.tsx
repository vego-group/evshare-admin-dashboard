...
function PromoDiscountTypeDropdown({
  value,
  setValue,
}: {
  ...
  const selectedLabel = discountTypeOptions.find((option) => option.value === value)?.label;
        ...
        <div className="absolute right-0 top-[calc(100%+4px)] z-30 w-full overflow-hidden rounded-[14px] border border-primary bg-bg-warm-ivory shadow-[0_10px_24px_rgba(16,24,40,0.12)]">
          {discountTypeOptions.map((option) => (
            <button
              ...
              type="button"
              onClick={() => {
                setValue("discount_type", option.value, { shouldDirty: true, shouldValidate: true });
                setIsOpen(false);
              }}
              className={cn(
...
export default PromoDiscountTypeDropdown;
