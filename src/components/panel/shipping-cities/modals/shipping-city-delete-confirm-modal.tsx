...
type ShippingCityDeleteConfirmModalProps = {
  cityName?: string;
  open: boolean;
  isDeleting: boolean;
  onClose: () => void;
...
function ShippingCityDeleteConfirmModal({
  cityName,
  open,
  isDeleting,
  onClose,
  onConfirm,
}: ShippingCityDeleteConfirmModalProps) {
  return (
    <Modal
      open={open}
      onClose={onClose}
      ...
      <div className="mx-auto flex w-full max-w-[481px] flex-col items-center justify-center gap-6 text-center">
        ...
        <div className="space-y-2">
          ...
          <p className="text-base font-medium leading-6 text-[#667085]">
            سيتم حذف ربط هذه المدينة مع مزود الشحن، ولن تتمكن من استرجاعه بعد
            تنفيذ هذا الإجراء.
          </p>
        </div>
...
export default ShippingCityDeleteConfirmModal;
