...
export default function PermissionsPanel() {
  ...
  return (
    <section className="space-y-4">
      <EntityToolbar title="الصلاحيات" description="إدارة صلاحيات النظام وربطها بالتصنيفات" search={search} orderBy={orderBy} onSort={(value) => { setOrderBy(value); setPage(1); }} onSearch={(value) => { setSearch(value); setPage(1); }} />
      <EntityTable rows={Array.isArray(data?.data) ? data.data : []} columns={columns} isLoading={isLoading} onView={setView} tableClassName="min-w-[680px]" />
