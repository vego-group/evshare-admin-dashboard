...
type CityDeleteConfirmModalProps = {
  cityName?: string;
  open: boolean;
  isDeleting: boolean;
  onClose: () => void;
...
function CityDeleteConfirmModal({
  cityName,
  open,
  isDeleting,
  onClose,
  onConfirm,
}: CityDeleteConfirmModalProps) {
  return (
    <Modal
      open={open}
      onClose={onClose}
      ...
      <div className="mx-auto flex w-full max-w-[481px] flex-col items-center justify-center gap-6 text-center">
        ...
        <div className="space-y-2">
          ...
          <p className="text-base font-medium leading-6 text-[#667085]">
            سيتم حذف هذه المدينة بشكل نهائي من النظام، ولن تتمكن من استرجاعها بعد تنفيذ هذا الإجراء.
          </p>
        </div>
...
export default CityDeleteConfirmModal;
