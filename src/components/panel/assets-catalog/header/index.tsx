...
import type { AssetsCatalogViewMode } from "@/types";

type AssetsCatalogHeaderProps = {
  viewMode: AssetsCatalogViewMode;
  onViewModeChange: (viewMode: AssetsCatalogViewMode) => void;
  onAddAsset: () => void;
};

function AssetsCatalogHeader({
  viewMode,
  onViewModeChange,
  onAddAsset,
}: AssetsCatalogHeaderProps) {
  return (
    <section className="flex w-full flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
      <Header
        title="كتالوج الأصول"
        subtitle="إدارة ومتابعة جميع الأصول الكهربائية"
      />
...
export default AssetsCatalogHeader;
