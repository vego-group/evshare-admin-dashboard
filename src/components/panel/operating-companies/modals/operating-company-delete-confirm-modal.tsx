...
type OperatingCompanyDeleteConfirmModalProps = {
  companyName?: string;
  open: boolean;
  isDeleting: boolean;
  onClose: () => void;
...
function OperatingCompanyDeleteConfirmModal({
  companyName,
  open,
  isDeleting,
  onClose,
  onConfirm,
}: OperatingCompanyDeleteConfirmModalProps) {
  return (
    <Modal
      open={open}
      onClose={onClose}
      ...
      <div className="mx-auto flex w-full max-w-120.25 flex-col items-center justify-center gap-6 text-center">
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-medium leading-8 text-[#344054]">
            هل أنت متأكد أنك تريد حذف شركة{" "}
            <span className="inline-block max-w-55 align-bottom truncate sm:max-w-[320px]">
              {companyName ?? ""}
            </span>
            ؟
          </h2>
          <p className="text-base font-medium leading-6 text-[#667085]">
            سيتم حذف هذه الشركة بشكل نهائي من النظام، ولن تتمكن من استرجاعها
            بعد تنفيذ هذا الإجراء.
          </p>
        </div>
...
export default OperatingCompanyDeleteConfirmModal;
