...
function ContractReviewModal({ vehicle, isSaving, open, onClose, onSaved, setIsSaving }: Props) {
  ...
  async function submit() {
    ...
    const result = await reviewVehicleContractAPI(vehicle.id, {
      status,
      rejection_reason: status === "contract_rejected" ? reason.trim() : undefined,
    });
    setIsSaving(false);
    if (result?.ok) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={`مراجعة عقد ${vehicleTitle(vehicle)}`} contentClassName="max-w-xl">
      <div className="space-y-4 p-1">
        <div className="rounded-[12px] bg-primary/5 p-3">
          <p className="mb-2 text-sm font-medium text-secondary">المرفقات</p>
          {attachments.length ? attachments.map((attachment, index) => <Attachment key={index} attachment={attachment} index={index} />) : <p className="text-sm text-gray">لا توجد مرفقات</p>}
        </div>
        <ReviewStatusDropdown value={status} onChange={setStatus} />
        {status === "contract_rejected" && (
          <textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder="اكتب سبب الرفض" className="min-h-28 w-full resize-none rounded-[12px] border border-primary bg-primary/4 p-3 outline-none" />
        )}
        <div className="flex justify-end gap-2">
          <Button type="button" variant="outline" onClick={onClose} disabled={isSaving}>إلغاء</Button>
          <Button type="button" onClick={submit} disabled={isSubmitDisabled} className="min-w-28 bg-primary text-secondary hover:bg-primary/90">
...
function Attachment({ attachment, index }: { attachment: NonNullable<VehicleListItem["vehicle_contract"]>["attachments"][number]; index: number }) {
  const url = typeof attachment === "string" ? attachment : (attachment as { url?: string }).url;
  const label = typeof attachment === "string" ? `مرفق ${index + 1}` : (attachment as { name?: string; file_name?: string }).name || (attachment as { file_name?: string }).file_name || `مرفق ${index + 1}`;
  return url ? <a href={url} target="_blank" rel="noreferrer" className="block text-sm font-medium text-secondary underline">{label}</a> : <p className="text-sm text-dark-gray">{label}</p>;
}
