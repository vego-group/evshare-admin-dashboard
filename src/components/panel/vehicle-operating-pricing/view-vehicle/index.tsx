...
import { useVehicle } from "@/hooks/api";
...
function ViewVehicle() {
  ...
  const { data, isLoading } = useVehicle(id ?? null);
    ...
    <div className="flex w-full flex-col gap-6">
      <div className="flex items-center gap-3">
        ...
        <Header
          ...
          subtitle="عرض بيانات المركبة المختارة"
        />
      </div>

      <div className="rounded-2xl border border-neutral-100 bg-white p-4 shadow-sm md:p-6">
        <VehicleDetailsContent vehicle={vehicle} isLoading={isLoading} />
      </div>
    </div>
  );
}
