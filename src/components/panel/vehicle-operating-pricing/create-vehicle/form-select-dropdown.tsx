  ...
  return (
    <div ref={rootRef} className="relative w-full">
      ...
      <button
        ...
        className={cn(
          "flex h-12 w-full items-center justify-between gap-3 rounded-xl border border-primary bg-primary/4 px-4 text-sm font-medium text-dark-gray outline-none transition hover:bg-primary/10 focus-visible:ring-2 focus-visible:ring-primary/30 disabled:cursor-not-allowed disabled:opacity-60",
          isOpen && "bg-primary/10",
        )}
      >
        <span className={cn("min-w-0 truncate text-right", !selectedOption && "text-gray-400")}>
          ...
          <div className="dashboard-dropdown-scroll">
            {options.length ? options.map((option) => (
              <button
                ...
                className={cn(
                  "flex min-h-11 w-full items-center justify-between gap-3 px-4 py-2 text-right text-sm font-medium text-dark-gray transition hover:bg-primary/10",
