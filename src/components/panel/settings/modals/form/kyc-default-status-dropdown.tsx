...
import type { KycDefaultStatusValue } from "@/types";
...
const statusOptions: Array<{ label: string; value: KycDefaultStatusValue }> = [
  { label: "قيد المراجعة", value: "pending" },
  { label: "موافق عليه", value: "approved" },
];

function KycDefaultStatusDropdown({ value, setValue }: Props) {
    ...
    <div ref={containerRef} className="relative mt-2 h-12 w-full">
      ...
      >
        <span className="text-right">
          {selectedOption?.label ?? "اختر الحالة الافتراضية"}
        </span>
        <ChevronDown
...
export default KycDefaultStatusDropdown;
