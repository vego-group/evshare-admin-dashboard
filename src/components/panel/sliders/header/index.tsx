...
export type SlidersViewMode = "table" | "card";

type SlidersHeaderProps = {
  viewMode: SlidersViewMode;
  onViewModeChange: (viewMode: SlidersViewMode) => void;
  onAddSlider: () => void;
};

function SlidersHeader({ viewMode, onViewModeChange, onAddSlider }: SlidersHeaderProps) {
  return (
    <section className="flex w-full flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
      <Header title="السلايدرات" subtitle="إدارة ومتابعة سلايدرات الصفحة الرئيسية" />
      ...
      <div className="flex items-center gap-3 self-start sm:self-auto">
        ...
        <PermissionGate slug="Admin Add Slides"><Button
          type="button"
          onClick={onAddSlider}
