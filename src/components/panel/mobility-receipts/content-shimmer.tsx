...
function MobilityReceiptsContentShimmer() {
  return (
    <section className="space-y-6" aria-hidden="true">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={index} className="flex items-center justify-between gap-3 rounded-[14px] border border-[#e5e7eb] bg-white px-5 py-5">
      ...
      <div className="overflow-hidden rounded-lg bg-white">
        <div className="min-w-[900px]">
          <div className="grid grid-cols-7 gap-4 border-b border-neutral-100 bg-neutral-50 px-5 py-4">
            {Array.from({ length: 7 }).map((_, index) => <Shimmer key={index} className="h-5 w-24 rounded-md" />)}
          </div>
          {Array.from({ length: 5 }).map((_, rowIndex) => (
            <div key={rowIndex} className="grid h-16 grid-cols-7 items-center gap-4 border-b border-neutral-100 px-5 py-3">
              {Array.from({ length: 7 }).map((_, cellIndex) => <Shimmer key={cellIndex} className="h-5 w-28 rounded-md" />)}
            </div>
          ))}
        </div>
      </div>
...
export default MobilityReceiptsContentShimmer;
